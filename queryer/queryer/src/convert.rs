/// 将 sqlparser 的结构转换为 polars 的 data frame, 为此， 实现一堆 TryFrom
...
impl TryFrom<Expression> for Expr {
    ...
    fn try_from(expr: Expression) -> Result<Self, Self::Error> {
        match *expr.0 {
            ...
            SqlExpr::IsNull(expr) => {
                Ok(<Expression as TryInto<Expr>>::try_into(Expression(expr))?.is_null())
            }
            SqlExpr::IsNotNull(expr) => {
                Ok(<Expression as TryInto<Expr>>::try_into(Expression(expr))?.is_null())
            }
            SqlExpr::Identifier(id) => Ok(Self::Column(id.value.into())),
    ...
    fn try_from(p: Projection<'a>) -> Result<Self, Self::Error> {
        match p.0 {
            ...
            SelectItem::ExprWithAlias {
                ...
                Arc::new(Expr::Column(id.to_string().into())),
